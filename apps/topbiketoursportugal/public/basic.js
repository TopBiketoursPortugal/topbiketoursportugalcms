(function () {
  ...
  const onLoad = function () {
    ...
    attachEvent('[data-aw-toggle-menu]', 'click', function (_, elem) {
      // Make sure the header is on-screen before it expands into a panel.
    ...
    function applyHeaderStylesOnScroll() {
      ...
      const y = lastKnownScrollPosition;
    ...
    // on `document` (which persists), so bind it only ONCE — otherwise
    // duplicate handlers with stale direction baselines fight each other.
    if (!window.__tbtHeaderScrollBound) {
      window.__tbtHeaderScrollBound = true;
