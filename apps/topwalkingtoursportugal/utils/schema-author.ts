 ...
 *   - the name of a group ("Top Walking Tours Portugal Editorial Team")
 *   - an English byline ("Written by Sérgio Marques, Founder & Route Designer,
 *     Top Walking Tours Portugal")
...
/** Site name, lowercased and regex-escaped, for matching the employer in a byline. */
const SITE_NAME_PATTERN = SiteData.en.site.name
  .toLowerCase()
  .replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
...
const NON_PERSON_AUTHORS = [
  ...
  /^redactie(team)?$/i,
  new RegExp(`^${SITE_NAME_PATTERN}$`, 'i'),
  /^admin(istrator)?$/i,
  /^author$/i
];
...
const EMPLOYER = new RegExp(`^${SITE_NAME_PATTERN}$`, 'i');

/**
 * A role can end with the connector that introduced the employer — "fundador y
 * diseñador de rutas **de** Top Walking Tours Portugal" leaves a trailing "de"
...
/** The employer name, with its optional leading connector, trailing a role. */
const EMPLOYER_SUFFIX = new RegExp(
  `[\\s,]*(?:de|da|do|du|van|von|of)?\\s*${SITE_NAME_PATTERN}\\s*$`,
  'i'
);
 ...
 * bylines have no comma to split on ("Sérgio Marques Founder of Top Walking
 * Tours Portugal"), and a known name is a far more reliable boundary than
 * guessing at where a job title starts.
 */
