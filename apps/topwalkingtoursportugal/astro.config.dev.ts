...
export default defineConfig({
  ...
  integrations: [
    ...
    sitemap({
      ...
      i18n: {
        defaultLocale: DEFAULT_LOCALE, // All urls that don't contain `pt` after `https://topwalkingtoursportugal.com/` will be treated as default locale, i.e. `en`
  ...
  env: {
    schema: {
      ...
      // TODO(walking): these defaults are copied verbatim from the old
      // walking site's astro.config.ts, but they are IDENTICAL to
      // topbiketoursportugal.com's ids (see apps/topbiketoursportugal's
      // astro.config.ts) — the old walking config was never customised with
      // its own GTM container / GA4 property. Confirm the real ids before
      // launch, or analytics for this site will land in the bike property.
    ...
    server: {
      watch: {
        ignored: [
          '**/google-cloud-sdk/**',
          '**/dist/**',
          '**/topwalkingtoursportugal.com-audit/**',
          '**/migration/**',
          '**/DataForSeo/**',
          '**/.astro/**',
          '**/node_modules/.vite/**'
