---
wetravel:
  uid: '2317568'
  selfGuidedUuid: '4640319057'
_schema: default
id: 6e6d1135-fa8d-4149-93fc-02152fa26f13
order: 1
title: Caminho de Santiago em Bicicleta - Caminho da Costa - "fácil"
content: >-
  # Caminho de Santiago em Bicicleta


  ## Caminho da Costa - "fácil"


  O caminho de Santiago pela costa apresenta-se como uma alternativa fácil,
  quando comparado com os percursos Portugueses mais tradicionais. O destino
  final permanece o mesmo, porém os desafios estão assim ao alcance da grande
  maioria dos ciclistas.


  Antes do início da peregrinação até Santiago de Compostela, recomendamos uma
  caminhada pela cidade do Porto. Ficará assim a conhecer um pouco da sua
  arquitectura, do seu património e tradições locais.


  Com o decorrer desta jornada até Santiago, passe por diversos destinos de
  muito sol e praia, como é o caso de Esposende, Caminha e Bayona. Desfrute das
  famosas ostras de Arcade e entenda porque Caldas de Reis detém relevância
  nobiliárquica.

  Por fim atingiremos a Praça de Obradoiro, onde se ergue a colossal catedral
  barroca que oferece sepultura ao restos mortais do apóstolo Santiago.


  **Descrição da rota:**


  * Rota plana durante os três primeiros dias com poucas subidas suaves e
  curtas.
    Montanhoso nos três últimos dias por estradas secundárias.
  * 30 % ciclovias e passadiços, 40 % caminhos em terra suave. Algumas ruas em
  paralelo 5% e caminhos em terra irregulares. 25 % estradas pavimentadas com
  pouco tráfego.

  * Elevação total: +3.043 m / -2.989 m ao longo de 6 dias.


  **Itinerário 7 noites / 8 dias::**


  * Dia 1: Recepção no Porto

  * Dia 2: Porto – Esposende (60 km)

  * Dia 3: Esposende – Caminha (50 km)

  * Dia 4: Caminha – Baiona (40 km)

  * Dia 5: Baiona – Pontevedra (64 km)

  * Dia 6: Pontevedra – Caldas dos Reis (26 km)

  * Dia 7: Caldas dos Reis – Santiago de Compostela (40 km)


  **Itinerário 8 noites / 9 dias:**


  * Dia 1: Recepção no Porto

  * Dia 2: Porto – Esposende (60 km)

  * Dia 3: Esposende – Caminha (50 km)

  * Dia 4: Caminha – Baiona (40 km)

  * Dia 5: Baiona – Vigo (25 km)

  * Dia 6: Vigo – Pontevedra (39 km)

  * Dia 7: Pontevedra – Caldas dos Reis (26 km)

  * Dia 8: Caldas dos Reis – Santiago de Compostela (40 km)
image:
  src: >-
    /src/assets/images/the-way-of-st.-james-bike-tour-fold-n-visit-cycling-holidays-3582-1280x853-1-.jpg
  alt: Top bike tours portugal
code: SANTIAGO 01
path: santiago-compostela-pela-costa
language: pt
tourtype: PackageTour
region: 8a7eeed3-22fe-4438-a69a-611497e0aefd
itinerary:
  - day: '1'
    description: >-
      Antes de ser iniciada a jornada até Santiago de Compostela, é quase
      obrigatório explorar o Porto, classificado como Património Mundial e
      Capital da Cultura e Turismo. Recomendamos um passeio a pé pela cidade
      para descobrir  parte do património que esta cidade tem para oferecer.
      Conheça lugares famosos como a livraria que influenciou o livro Harry
      Potter de J.K. Rowling, o magnífico mercado da bolsa, os azulejos azuis e
      brancos que se escondem a cada esquina e muito mais. Arte, património e
      tradições esperam por si ao longo deste pequeno passeio pela cidade.
      Alojamento, Hotel 4 \*: [Porto Trindade
      Hotel](https://www.portotrindadehotel.com/)
    location:
      lat: '41.15293'
      lng: '-8.61005'
    title: 'Dia 1: Recepção no Porto'
  - day: '2'
    description: >-
      niciará a sua viagem em direcção a Esposende a partir da nossa loja Top
      Bike Tours Portugal, sediada na Invicta cidade do Porto.  Durante este
      primeiro dia, verá inúmeras paisagens à beira-mar, assim como pequenas
      "rivieras portuguesas" Atravessará uma enorme variedade de vilas
      piscatórias, como é o caso da Póvoa de Varzim, onde se pode testemunhar
      antigas tradições ligadas à pesca e onde poderá saborear esta famosa
      gastronomia. Em Vila do Conde irá perceber a razão, pela qual estas
      cidades costeiras foram tão importantes durante o período áureo dos
      Descobrimentos Portugueses. Alojamento, Hotel 4 *: [Portas do Sol
      Hotel](https://hotelportadosol.com/)
    location:
      lat: '41.15591'
      lng: '-8.60248'
    title: 'Dia 2: Porto – Esposende (60 km)'
  - day: '3'
    description: >-
      Viana do Castelo estabelece-se como a principal cidade por onde irá
      passar. Esta serve como mais um belo exemplo de uma cidade de ancoragem à
      beira-mar, e com uma conotação ancestral e uma enorme importância durante
      os Descobrimentos. Durante os séculos XIV e XV, foi nesta cidade que foram
      desenvolvidos os navios, que partiram para o Brasil, Índia, África e
      outras colónias portuguesas.  O seu destino final será Caminha. Esta
      cidade encontra o delta do rio Minho, sendo parte de uma paisagem
      incrível. A cidade de Caminha é largamente conhecida pelo seu património
      natural, como as áreas de passeios, cachoeiras, praias e parque de
      campismo. Alojamento, Hotel 4 *: [Parador Baiona
      Hotel](https://www.parador.es/es/paradores/parador-de-baiona?utm_source=GoogleMyBusiness&utm_medium=linkgoogle&utm_campaign=paradordebaiona&utm_term=organico&utm_content=ficha)
    location:
      lat: '41.15591'
      lng: '-8.60248'
    title: 'Dia 3: Esposende – Caminha (50 km)'
  - day: '4'
    description: >-
      Neste dia vamos começar por atravessar o rio minho e desembarcar em
      território espanhol. O seu destino, Baiona, é uma cidade politicamente
      ligada a Pontevedra, onde se pode respirar a brisa do mar. Para além da
      paisagem marítima, poderá perder-se no antigo centro da cidade, nascido a
      partir de raízes romanas e que cresceu sempre em direcção ao litoral
      durante a Idade Média, com a influência da Igreja Católica. Curiosamente,
      durante o século XVI, Baiona foi invadida por navios britânicos liderados
      pelo grande corsário e explorador Francis Drake, que viu uma oportunidade
      para estabelecer um ponto comercial e mercantil neste território.
      Alojamento, Hotel 4 *: [Parador Baiona
      Hotel](https://www.parador.es/es/paradores/parador-de-baiona?utm_source=GoogleMyBusiness&utm_medium=linkgoogle&utm_campaign=paradordebaiona&utm_term=organico&utm_content=ficha)
    location:
      lat: '41.15591'
      lng: '-8.60248'
    title: 'Dia 4: Caminha - Baiona (40 Km)'
  - day: '5'
    description: >-
      Desde a ria de Vigo até à baía de San Simon terá uma paisagem marítima
      para regalar os olhos. Seguindo sempre a linha da costa, este dia será
      dedicado a Arcade e às suas famosas ostras. Não obstante esta deliciosa
      iguaria, poderá ainda ver monumentos e pontos turísticos dedicados à
      cultura de Santiago, como a igreja de Santiago em Arcade, dos séculos
      XII-XIII, e as estradas e caminhos que o levarão ao seu destino de
      peregrinação. É quase obrigatório passar por Arcade e apreciar as famosas
      ostras, produzidas e colhidas nas águas da região. Durante a primeira
      semana de abril ocorre um festival dedicado a esta gastronomia
      tradicional, a "Fiesta de la Ostra", onde a cidade se encontra repleta de
      turistas e peregrinos que vêm de longe para participar neste evento.
      Alojamento, Hotel 4 *: [Parador Pontevedra
      Hotel](https://www.parador.es/es/paradores/parador-de-pontevedra?utm_source=GoogleMyBusiness&utm_medium=linkgoogle&utm_campaign=paradordepontevedra&utm_term=organico&utm_content=ficha)
    location:
      lat: '41.15591'
      lng: '-8.60248'
    title: 'Dia 5: Baiona – Pontevedra (64 km)'
  - day: '6'
    description: >-
      De acordo com os arquivos históricos, Caldas de rei tem a sua origem numa
      comunidade pré-romana que vivia das fontes termais deste território. O seu
      nome está ligado ao local de nascimento de Afonso VII, altura em que
      actuava já como um caminho de peregrinação a Santiago de Compostela. Além
      da rica herança cultural, como Arcade, Caldas de Reis mantém uma
      gastronomia peculiar, tais como as "empanadas" de lampreia e do pão
      tradicional espanhol. Uma vez que as fontes de água quente acompanharam a
      evolução deste território, hoje em dia é um ponto de interesse turístico
      para quem procura tratamentos mais saudáveis para diversas doenças.
      Alojamento, Casa Rural: [Torre do Rio](https://torredorio.es/en/)
    location:
      lat: '41.15591'
      lng: '-8.60248'
    title: 'Dia 6: Pontevedra – Caldas dos Reis (26 km)'
  - day: '7'
    description: >-
      O último e sétimo dia será percorrido desde Caldas de Reis até Santiago de
      Compostela, o nosso destino final e um dos núcleos de peregrinação
      religiosa na Europa Ocidental.  Tal como Roma e Jerusalém, a cidade de
      Santiago de Compostela apresenta-se-nos como um ícone religioso único para
      os turistas, que encontram nesta viagem de ciclismo um acto de fé e de
      realização interna. Chegará à Praça de Obradoiro, onde terá a
      possibilidade de contemplar a majestosa catedral barroca, onde os restos
      mortais do apóstolo estão localizados. Alojamento, Hotel 4 *: [Compostela
      Hotel](https://www.hotelcompostela.es/es/)
    location:
      lat: '41.15591'
      lng: '-8.60248'
    title: 'Dia 7: Caldas dos Reis – Santiago de Compostela (40 km)'
  - day: '8'
    location:
      lat: '41.15591'
      lng: '-8.60248'
    title: 'Dia 8: Partida'
packages:
  - bestValue: false
    discount: 0
    highSeasonPriceSupplement: 100
    observations: ''
    title: Self Guided bike Tour
    description: Self Guided bike Tour
    included:
      - title: Accommodations W/ Breakfast
        description: Yes (4 stars, farms or rural houses)
      - title: Luggage shuttling
        description: 'Yes'
      - title: Uber Porto Airport Transfers
        description: 'Yes'
      - icon: ✓
        title: Trains or Bus Tickets
        description: 'Yes'
      - title: Van Support
        description: 'No'
      - title: GPS With route per booking
        description: 'Yes'
      - title: Bike rent
        description: 'Yes'
      - title: Guide
        description: 'No'
      - title: Mechanical Assistance
        description: 'No'
      - title: 6 dinners (Half-Board)
        description: extra 350€ (at your hotel)
      - title: 6 lunchs + 6 dinners (Full-board)
        description: X
      - title: Personal Accident Insurance
        description: 'No'
      - title: Liability Insurance
        description: 'No'
      - title: Min. Participants
        description: '2'
      - title: 7 Nights / 8 Days
        description: Price per person
      - title: '* Low season'
        description: 1.541€
      - title: '* Regular season'
        description: +200€
      - title: '* High Season'
        description: +266€
      - title: '* Summer Season'
        description: +332€
      - title: 8 Nights / 9 Days
        description: 300 €
      - title: Premium hotel upgrade supplement
        description: +650€
      - title: Top Bike Tours Portugal Jersey
        description: extra 45€
      - title: Drinks, snacks and experiences
        description: X
    price:
      bestValue: true
      currency: EUR
      price: 1541
      promo: 0
      amount: 0
    type: ''
  - bestValue: true
    discount: 0
    highSeasonPriceSupplement: 100
    title: Guided Bike tour
    description: Guided Bike tour
    included:
      - title: Accommodations W/ Breakfast
        description: Yes (4 stars, farms or rural houses)
      - title: Luggage shuttling
        description: 'Yes'
      - title: Uber Porto Airport Transfers
        description: 'Yes'
      - title: Trains or Bus Tickets
        description: 'Yes'
      - title: Van Support
        description: minimum 8 pax
      - icon: '-'
        description: '-'
        title: GPS With route per booking
      - title: Bike rent
        description: 'Yes'
      - title: Guide
        description: 'Yes'
      - title: Mechanical Assistance
        description: 'Yes'
      - title: 6 dinners (Half-Board)
        description: extra 350€
      - title: 6 lunchs + 6 dinners (Full-board)
        description: extra 650€
      - title: Personal Accident Insurance
        description: 'Yes'
      - title: Liability Insurance
        description: 'Yes'
      - title: Min. Participants
        description: '4'
      - title: 7 Nights / 8 Days
        description: Price per person
      - title: '* Low season'
        description: 2.297€
      - title: '* Regular season'
        description: +200€
      - title: '* High Season'
        description: +266€
      - title: '* Summer Season'
        description: +332€
      - title: 8 Nights / 9 Days
        description: X
      - title: Premium hotel upgrade supplement
        description: +650€
      - title: Top Bike Tours Portugal Jersey
        description: 'Yes'
      - title: Drinks, snacks and experiences
        description: X
    price:
      bestValue: false
      currency: EUR
      price: 2297
      promo: 0
      amount: 0
    type: ''
  - bestValue: true
    discount: 0
    highSeasonPriceSupplement: 100
    title: Guided Premium *
    description: Guided Premium\*
    included:
      - title: Accommodations W/ Breakfast
        description: Yes (4 stars, farms or rural houses)
      - title: Luggage shuttling
        description: 'Yes'
      - title: Uber Porto Airport Transfers
        description: 'Yes'
      - title: Trains or Bus Tickets
        description: 'Yes'
      - title: Van Support
        description: 'Yes'
      - icon: '-'
        description: '-'
        title: GPS With route per booking
      - title: Bike rent
        description: 'Yes'
      - title: Guide
        description: 'Yes'
      - title: Mechanical Assistance
        description: 'Yes'
      - title: 6 dinners (Half-Board)
        description: extra 650€
      - title: 6 lunchs + 6 dinners (Full-board)
        description: extra 950€
      - title: Personal Accident Insurance
        description: 'Yes'
      - title: Liability Insurance
        description: 'Yes'
      - title: Min. Participants
        description: '4'
      - title: 7 Nights / 8 Days
        description: Price per person
      - title: '* Low season'
        description: 3.397€
      - title: '* Regular season'
        description: +200€
      - title: '* High Season'
        description: +266€
      - title: '* Summer Season'
        description: +332€
      - title: 8 Nights / 9 Days
        description: X
      - title: Premium hotel upgrade supplement
        description: X
      - title: Top Bike Tours Portugal Jersey
        description: 'Yes'
      - title: Drinks, snacks and experiences
        description: Yes (post ride)
    price:
      bestValue: false
      currency: EUR
      price: 3397
      promo: 0
      amount: 0
    type: '1'
afterPricing: >-
  **\*Temporada:**


  * LS - Low season: Nov 01 to Mar 25

  * RS - Regular season: Mar 26 to 25 Jun and Set 26 to 31 Oct

  * HS - High Season: Set 01 to 25 Set

  * SS - Summer Season: Jun 26 to August 30


  **Observações:**


  * Preços baseados para 2 pessoas que partilham o mesmo quarto standard;

  * Quartos vista mar (quando disponiveis) ou quartos premium, preços sob
  consulta

  * Preços podem sofrer alterações devido a alta temporada, feriados ou fins de
  semana

  * Bebidas não incluídas ao almoço ou jantar quando incluídas no package

  * Se em quarto triplo com os pais Crianças até 10 anos - desconto de 35%.
  Crianças dos 11 aos 14 anos - 20% de desconto. Crianças dos 15 aos 17 anos -
  15% de desconto.


  **Extras**


  * Bicicleta Elétrica:: 300€ (350€ para 8 opção de 8 noites)

  * Suplemento para quarto individual: LS: 480€, RS: 600€, HS: 720€, SS.840€
  (Premium option acresce 950€)

  * Suplemento para quarto individual para opção de 8 noites n: LS: 550€, RS:
  670€, HS: 790€, SS.840€ (Premium option acresce 1.050€)

  * Viajante único: 200 €

  * Viajante único opção 8 noites: 250 €

  * Noites extra: sob consult

  * Preço para Grupo privado: sob consulta

  * Taxa turística não está incluída no preço do package (aproximadamente 2€ por
  pessoa por noite - pagos no local - em Lisboa a Taxa turística é de 4€ por
  pessoa por noite)

  * Ferry (barco) na travessia de rio/lago não está incluído no preço do package
  (aproximadamente 2€ a 10€ por pessoa)


  [**Saiba mais**](https://topbiketoursportugal.com/pt/bicicletas/) **sobre as
  nossas bicicletas.**
seo:
  page_title: Caminho de Santiago Pela Costa de Bicicleta - Top Bike Tours
  page_description: >-
    O Caminho de Santiado pela Costa de bicicleta, preparado pela Top Bike
    Tours, é mais simples do que os tradicionais portugueses, e promete muito
    Sol e praia!
  canonical_url:
  featured_image: >-
    /src/assets/images/the-way-of-st.-james-bike-tour-fold-n-visit-cycling-holidays-3582-1280x853-1-.jpg
  featured_image_alt: Caminho de Santiago Pela Costa de Bicicleta - Top Bike Tours
  author_twitter_handle:
  open_graph_type: product
  no_index: false
  keywords: >-
    passeio de bicicleta santiago de compostela, passeio guiado até santiago de
    compostela, bike tour porto a santiago de compostela
  description: >
    O Caminho de Santiado pela Costa de bicicleta, preparado pela Top Bike
    Tours, é mais simples do que os tradicionais portugueses, e promete muito
    Sol e praia!
  title: Caminho de Santiago Pela Costa de Bicicleta - Top Bike Tours
duration: 8
distance: 278
tags:
  - 2a4b8647-2f93-494f-a4ea-ecff8b07c000
  - c0fbc7fd-377c-415d-b265-4cba05c9a887
  - a973ec6d-511a-4f16-908b-ec09dc5d3f8c
minGroupSize: 1
maxGroupSize: 1
difficulty: 3
skill: 1
minAge: 2
mapUrl: https://www.google.com/maps/d/embed?mid=1xFumA-MEafLJTH8FdgLg6iTTmAnYVEc9
images:
  - src: /src/assets/images/camino-de-santiago-coastal-route-bicycle-tour.jpg
    alt: group 1
  - src: >-
      /src/assets/images/the-way-of-st.-james-bike-tour-top-bike-tours-portugal.jpg
    alt: group 2
  - src: /src/assets/images/crossing-the-ferry-with-bicycles.jpg
    alt: CROSSING THE FERRY WITH BICYCLES
  - src: /src/assets/images/biketour-group-photo-in-viana-do-castelo.jpg
    alt: GROUP PHOTO VIANA DO CASTELO WITH TOP BIKE TOURS PORTUGAL
  - src: /src/assets/images/biking-on-the-camino-trail.jpg
    alt: BIKING ON THE CAMINO TRAIL
  - src: /src/assets/images/electric-bicycle-on-the-camino.jpg
    alt: ELECTRIC BICYCLE ON THE CAMINO
  - src: /src/assets/images/group-photo-in-the-water-fall.jpg
    alt: GROUP PHOTO IN THE WATER FALL
  - src: /src/assets/images/mountain-bicycle-on-the-camino.jpg
    alt: GROUP PHOTO IN THE WATER FALL
  - src: >-
      /src/assets/images/the-way-of-st.-james-bike-tour-fold-n-visit-cycling-holidays-4394.jpg
    alt: The Way of St. James Bike Tour - Fold N Visit Cycling Holidays-4394
faqs:
  - question: '**O que torna o percurso Porto–Santiago especial?**'
    answer: >-
      Este percurso segue o histórico **Caminho de Santiago**, começando na bela
      cidade costeira do **Porto**, em Portugal, e terminando na famosa
      **Catedral de Santiago de Compostela**, em Espanha. Ao longo do caminho,
      poderá desfrutar de uma mistura rica de **paisagens costeiras, vilas
      piscatórias encantadoras, cidades medievais, vinhas e campos
      verdejantes**. É uma combinação perfeita de **descoberta cultural, beleza
      natural e herança espiritual**.
  - question: '**Quais são os locais visitados ao longo do percurso?**'
    answer: >-
      **Dia 1 – Receção no Porto**


      Bem-vindo ao **Porto**, uma cidade Património Mundial da UNESCO, cheia de
      charme e história. Aproveite para explorar o colorido bairro da Ribeira,
      admirar a icónica **Ponte D. Luís I** e saborear um copo de vinho do Porto
      numa das caves tradicionais de **Vila Nova de Gaia**. As bicicletas serão
      preparadas e receberá um briefing detalhado sobre a aventura que o espera.


      **Dia 2 – Porto → Esposende (≈ 60 km)**


      A sua jornada começa ao longo da **Costa Atlântica**, seguindo passadiços
      de madeira e trilhos costeiros que passam por **Matosinhos, Vila do
      Conde** e **Póvoa de Varzim**. Desfrute da brisa do mar e pare para um
      café numa das vilas piscatórias ao longo do caminho. O dia termina em
      **Esposende**, uma tranquila vila à beira do rio Cávado.


      **Dia 3 – Esposende → Caminha (≈ 50 km)**


      O percurso segue para norte, passando por **Viana do Castelo**, uma das
      cidades mais bonitas de Portugal, famosa pelo **Santuário de Santa
      Luzia**, com vistas deslumbrantes sobre a cidade e o mar. A paisagem
      alterna entre praias, dunas e campos verdes até chegar a **Caminha**, uma
      pitoresca vila fronteiriça junto ao rio Minho, onde Portugal encontra
      Espanha.


      **Dia 4 – Caminha → Baiona (≈ 40 km)**


      Atravessa-se o **rio Minho de ferry** para entrar em Espanha, na região da
      **Galiza**. O trajeto segue por estradas costeiras tranquilas, passando
      por **A Guarda**, conhecida pelo seu antigo castro celta (**Castro de
      Santa Tegra**) e pelas vistas panorâmicas sobre o Atlântico. Continua-se
      pela costa, passando pelo **Mosteiro de Oia**, até chegar a **Baiona**,
      uma animada vila marítima cheia de história.


      **Dia 5 – Baiona → Pontevedra (≈ 64 km)**


      O dia começa com o trajeto em direção a **Vigo**, a maior cidade da
      Galiza, onde poderá fazer uma pausa para tapas e desfrutar do ambiente
      junto ao mar. O percurso segue depois por caminhos interiores até
      **Pontevedra**, uma das cidades mais encantadoras da Galiza, conhecida
      pelo seu centro histórico pedonal, praças de pedra e cafés acolhedores.


      **Dia 6 – Pontevedra → Caldas de Reis (≈ 26 km)**


      Um dia mais curto, ideal para relaxar e apreciar a beleza rural da Galiza.
      Pedale por trilhos florestais e pequenas aldeias até chegar a **Caldas de
      Reis**, uma vila termal famosa pelas suas **águas quentes naturais** —
      perfeitas para recuperar energia depois de uma semana de pedaladas.


      **Dia 7 – Caldas de Reis → Santiago de Compostela (≈ 40 km)**


      A última etapa passa por **Padrón**, a lendária vila onde, segundo a
      tradição, chegaram as relíquias do apóstolo Santiago. A partir daí, uma
      subida suave conduz até à **imponente Catedral de Santiago de
      Compostela**. Celebre a sua chegada na **Praça do Obradoiro**, absorva o
      ambiente de peregrinação e desfrute da sensação de ter completado uma das
      viagens de bicicleta mais icónicas da Europa.
  - question: '**Qual é o nível de dificuldade deste tour?**'
    answer: >-
      O percurso de Porto a Santiago tem **dificuldade moderada**. Inclui uma
      combinação de **ciclovias costeiras planas**, **zonas rurais onduladas** e
      **algumas subidas mais desafiantes**, especialmente ao aproximar-se da
      Galiza. As distâncias diárias variam, mas os participantes devem sentir-se
      confortáveis a pedalar **entre 40 e 70 km por dia**.
  - question: '**É seguro pedalar nas cidades portuguesas?**'
    answer: >-
      Sim, os nossos percursos são cuidadosamente planeados para serem seguros e
      agradáveis, com trajetos tranquilos e vistas incríveis.
  - question: '**É necessário ter experiência em ciclismo?**'
    answer: >-
      Não! Temos tours para todos os níveis, desde iniciantes até ciclistas
      experientes. Antes de cada passeio, o guia adapta o percurso ao ritmo e
      experiência do grupo.
  - question: '**Como posso reservar um tour?**'
    answer: >-
      As reservas podem ser feitas diretamente através do nosso website, por
      e-mail
      [reservations@topbiketoursportugal.com](mailto:reservations@topbiketoursportugal.com)
      ou telefone [(+351) 915 316 999](tel:+351915316999).


      Recebera uma confirmação assim que o pagamento for efetuado.


      &nbsp;
  - question: '**O que devo levar para o tour?**'
    answer: >-
      Recomendamos o uso de roupa confortável, protetor solar, água e um pequeno
      lanche. Fornecemos a bicicleta, pedais, capacete, porta-bagagens traseiro,
      alforje traseiro, câmaras de ar suplentes, garrafa de água, bomba e
      ferramentas.
  - question: >-
      **Quanto do percurso é em estrada asfaltada e quanto é em caminhos de
      terra?**
    answer: >-
      O tour combina **caminhos asfaltados e trilhos não pavimentados**. A
      proporção varia de dia para dia, e o detalhe de cada etapa está indicado
      no itinerário diário.
  - question: '**São fornecidos alforges?**'
    answer: Sim. Cada participante recebe **um alforge traseiro**
relatedTours:
  - cb799472-4882-4fe2-9e85-cf43ab2b5a0e
  - 173c8114-8bb9-42a1-b69a-dc159ef8617e
riderLevels:
  - d371536f-33f8-4e1d-818c-035dcf527ea9
templateKey: tour-gen
featured: true
packagetype: PackageTour
tourcategory:
  - Bike
groupSizeMax: 1
groupSizeMin: -2
skillLevel: 3
distanceUnit: Km
durationUnit: Dias | 7 Noites
physicality: 1
afterpricing: >-
  **\*Temporada:**


  * LS - Low season: Nov 01 a Mar 25

  * RS - Regular season: Mar 26 a 25 Jun e Set 26 a 31 Oct

  * HS - High Season: Set 01 a 25 Set

  * SS - Summer Season: Jun 26 a August 30


  **Observações:**


  * Preços baseados para 2 pessoas que partilham o mesmo quarto; standard

  * Quartos vista mar (quando disponiveis) ou quartos premium, preços sob
  consulta

  * Preços podem sofrer alterações devido a alta temporada, feriados ou fins de
  semana

  * Bebidas não incluídas ao almoço ou jantar quando incluídas no package

  * Se em quarto triplo com os pais Crianças até 10 anos - desconto de 35%.
  Crianças dos 11 aos 14 anos - 20% de desconto. Crianças dos 15 aos 17 anos -
  15% de desconto.


  **Extras**


  * Bicicleta Elétrica: 270€ (315 € no Package de 8 noites)

  * Suplemento para quarto individual: 390€, RS: 420€, HS: 450€, SS.480€

  * Suplemento para quarto individual opção 8 noites: 445€, RS: 475€, HS: 505€,
  SS.535€

  * Viajante único: 200 €

  * Viajante único opção 8 noites: 220 €

  * Noites extra: sob consulta

  * Preço para Grupo privado: sob consulta

  * Taxa turística não está incluída no preço do package (aproximadamente 2€ por
  pessoa por noite - pagos no local - em Lisboa a Taxa turística é de 4€ por
  pessoa por noite)

  * Ferry (barco) na travessia de rio/lago não está incluído no preço do package
  (aproximadamente 2€ a 10€ por pessoa)
feature:
  align: center
  image: /src/assets/images/furadouro-forest-in-a-bicycle.jpg
  imageAlign: center
  subtitle: Passeios com guia e em autonomia em Portugal
  textcolor: white
  title: Passeios com guia e em autonomia em Portugal
bikeCategories:
  - f0a8d9e4-a708-43e3-9171-dd3472e29d71
  - eb6db1c0-c5c6-4292-9d77-f9fb7476e75c
  - 50ca4b9c-db87-43f6-8e07-08feb8db3dbd
  - 6d84d925-64dc-45ce-bd0f-088ee603cad5
---
