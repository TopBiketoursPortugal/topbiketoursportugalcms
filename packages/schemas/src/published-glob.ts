 ...
 *
 * A draft here is a translation stub: the mdx-clone tool drops one into each
 * sibling language folder (`pt/`, `de/`, …) whenever an English entry is
 * added, so an editor has somewhere to write the translation instead of
 * having to remember to create the file. Until that happens the stub still
 * holds English prose, and publishing it would put English text on a
 * translated URL and — since pages and sitemap pair translations by filename
 * — declare it the translation of its own original.
 *
 ...
 * could not simply be extended. The SEO tooling's routes.mjs applies the same
 * rule to the content tree it reads for the sitemap and redirect tooling.
 *
 * Paths are resolved against `process.cwd()`, which is the app directory when
 * `astro build` runs through `pnpm --filter <app>`.
 */
