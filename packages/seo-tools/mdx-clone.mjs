...
// The app directory — this runs through the app's `pnpm mdx-clone` script.
...
function prepareStub(file, language) {
  ...
  // The source may carry its own `draft:` line; a stub is unpublished regardless.
  updated = /^draft:/m.test(updated)
    ? updated.replace(/^draft:.*$/m, 'draft: true')
    : updated.replace(/^---\r?\n/, '---\ndraft: true\n');
