...
// The canonical origin, from the app's own site data — the same value
// astro.config.ts builds absolute URLs from.
const SITE = siteOrigin();

function siteOrigin() {
  const site = JSON.parse(
    readFileSync(join(REPO_ROOT, 'data/site.json'), 'utf8')
  );
  const url =
    Object.values(site).find((l) => l?.site?.url)?.site?.url ??
    process.env.SITE_URL;
  if (!url) {
    console.error('✗ no site URL in data/site.json (<lang>.site.url).');
    process.exit(1);
  }
