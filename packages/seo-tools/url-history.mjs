...
function historicalFieldValues() {
  // Git prints repository-relative paths; the app's content is addressed
  // app-relative everywhere else, so strip the app's prefix as we go. A file
  // that moved with the app into the monorepo (`src/content/x.mdx` ->
  // `apps/<app>/src/content/x.mdx`) then keeps one identity across the move.
  const log = git([
    'log',
    '-p',
    '-M',
  ...
  for (const line of log.split('\n')) {
    ...
    if (line.startsWith('diff --git ')) {
      ...
      file = match ? stripGitPrefix(match[2]) : null;
      renameFrom = null;
      continue;
    }
    ...
    if (line.startsWith('rename to ') && renameFrom) {
      const renameTo = stripGitPrefix(line.slice('rename to '.length));
      if (renameTo !== renameFrom) renames.set(renameFrom, renameTo);
      renameFrom = null;
      continue;
    }
