var e=class extends HTMLElement{shadowRoot;iframeLoaded=!1;domRefFrame;domRefImg;domRefPlayButton;hash=void 0;static preconnected=!1;constructor(){super()}static get observedAttributes(){return[`videoid`]}get isUnlisted(){return this.hasAttribute(`unlisted`)?this.getAttribute(`unlisted`)===`true`||this.getAttribute(`unlisted`)===``:!1}get enableTracking(){return this.hasAttribute(`enabletracking`)}get hasCustomPlaceholder(){return this.hasAttribute(`customplaceholder`)}get customPlaceholder(){return this.getAttribute(`customplaceholder`)||``}get videoId(){let e=this.getAttribute(`videoid`);if(!e)return``;if(this.isUnlisted){let[t,n]=e.split(`/`);return this.hash=n,t}return e}get loop(){return this.hasAttribute(`loop`)}set videoId(e){this.setAttribute(`videoid`,e)}get videoPlay(){return this.getAttribute(`videoplay`)||`Play`}set videoPlay(e){this.setAttribute(`videoplay`,e)}get videoTitle(){return this.getAttribute(`videotitle`)||`Video`}set videoTitle(e){this.setAttribute(`videotitle`,e)}get videoStartAt(){return this.getAttribute(`start`)||`0s`}set videoStartAt(e){this.setAttribute(`start`,e)}get autoLoad(){return this.hasAttribute(`autoload`)}set autoLoad(e){e?this.setAttribute(`autoload`,``):this.removeAttribute(`autoload`)}get autoPlay(){return this.hasAttribute(`autoplay`)}set autoPlay(e){e?this.setAttribute(`autoplay`,`autoplay`):this.removeAttribute(`autoplay`)}},t=class extends e{constructor(){super()}addStyles(){return`<style>
        :host {
          contain: content;
          display: block;
          position: relative;
		...
		<button class="lvo-playbtn"></button>
      	</div>`}},n=class e extends t{constructor(){super(),this.setupDom()}setupDom(){this.shadowRoot?this.shadowRoot.innerHTML=``:this.shadowRoot=this.attachShadow({mode:`open`}),this.shadowRoot.innerHTML=this.addStyles()+this.addPictureElement(),this.domRefFrame=this.shadowRoot.querySelector(`#frame`),this.domRefImg={fallback:this.shadowRoot.querySelector(`#fallbackPlaceholder`),webp:this.shadowRoot.querySelector(`#webpPlaceholder`),jpeg:this.shadowRoot.querySelector(`#jpegPlaceholder`)},this.domRefPlayButton=this.shadowRoot.querySelector(`.lvo-playbtn`)}connectedCallback(){this.shadowRoot||=this.attachShadow({mode:`open`}),this.addEventListener(`pointerover`,e.warmConnections,{once:!0}),this.addEventListener(`click`,()=>this.addIframe())}addIframe(){if(!this.iframeLoaded){let e=this.getIFrameParams(),t=`/video/${this.videoId}${this.isUnlisted?`?h=${this.hash}&${e}`:`?${e}`}`,n=new URL(t,`https://player.vimeo.com/`),r=`
			<iframe frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope" allowfullscreen autoplay="true" ${this.autoPlay?`muted="true"`:``} src="${n}"></iframe>`;this.domRefFrame.insertAdjacentHTML(`beforeend`,r),this.domRefFrame.classList.add(`lvo-activated`),this.iframeLoaded=!0}}getIFrameParams(){let e=`hd=1&autohide=1&autoplay=1`;return e+=this.loop?`&loop=1`:``,e+=this.enableTracking?``:`&dnt=1`,e+=this.autoPlay?`&muted=1`:``,e+=this.videoStartAt?`&#t=${this.videoStartAt}`:``,e}setupComponent(){this.initImagePlaceholder(),this.setAttribute(`title`,`${this.videoPlay}: ${this.videoTitle}`),this.domRefPlayButton.setAttribute(`aria-label`,`${this.videoPlay}: ${this.videoTitle}`),this.autoLoad&&this.initIntersectionObserver()}attributeChangedCallback(e,t,n){e===`videoid`&&t!==n&&(this.setupComponent(),this.domRefFrame.classList.contains(`lvo-activated`)&&(this.domRefFrame.classList.remove(`lvo-activated`),this.shadowRoot?.querySelector(`iframe`).remove()))}initIntersectionObserver(){`IntersectionObserver`in window&&`IntersectionObserverEntry`in window&&new IntersectionObserver((t,n)=>{t.forEach(t=>{t.isIntersecting&&!this.iframeLoaded&&(e.warmConnections(),this.addIframe(),n.unobserve(this))})},{root:null,rootMargin:`0px`,threshold:0}).observe(this)}initImagePlaceholder=async()=>{if(this.isUnlisted)return;e.preconnected||e.addPrefetch(`preconnect`,`https://i.vimeocdn.com/`);let t=`https://vimeo.com/api/v2/video/${this.videoId}.json`,n=(await(await fetch(t)).json())[0].thumbnail_large,r=n.substr(n.lastIndexOf(`/`)+1).split(`_`)[0],i=`https://i.vimeocdn.com/video/${r}.webp?mw=1100&mh=619&q=70`,a=`https://i.vimeocdn.com/video/${r}.jpg?mw=1100&mh=619&q=70`;this.domRefImg.webp.srcset=i,this.domRefImg.jpeg.srcset=a,this.domRefImg.fallback.src=a,this.domRefImg.fallback.setAttribute(`aria-label`,`${this.videoPlay}: ${this.videoTitle}`),this.domRefImg.fallback.setAttribute(`alt`,`${this.videoPlay}: ${this.videoTitle}`)};static addPrefetch(e,t,n=null){let r=document.createElement(`link`);r.rel=e,r.href=t,n&&(r.as=n),r.crossOrigin=`true`,document.head.append(r)}static warmConnections(){e.preconnected||=(Object.entries({preconnect:[`https://f.vimeocdn.com`,`https://player.vimeo.com`,`https://i.vimeocdn.com`]}).forEach(([t,n])=>{n.forEach(n=>e.addPrefetch(t,n))}),!0)}};try{customElements.get(`lite-vimeo`)||customElements.define(`lite-vimeo`,n)}catch(e){console.error(e)}var r=class e extends HTMLElement{constructor(){super(),this.isIframeLoaded=!1,this.isPlaylistThumbnailLoaded=!1,this.setupDom()}static get observedAttributes(){return[`videoid`,`playlistid`,`videoplay`,`videotitle`]}connectedCallback(){this.addEventListener(`pointerover`,()=>e.warmConnections(this),{once:!0}),this.addEventListener(`click`,()=>this.addIframe())}get videoId(){return encodeURIComponent(this.getAttribute(`videoid`)||``)}set videoId(e){this.setAttribute(`videoid`,e)}get playlistId(){return encodeURIComponent(this.getAttribute(`playlistid`)||``)}set playlistId(e){this.setAttribute(`playlistid`,e)}get videoTitle(){return this.getAttribute(`videotitle`)||`Video`}set videoTitle(e){this.setAttribute(`videotitle`,e)}get videoPlay(){return this.getAttribute(`videoplay`)||`Play`}set videoPlay(e){this.setAttribute(`videoplay`,e)}get videoStartAt(){return this.getAttribute(`videoStartAt`)||`0`}get autoLoad(){return this.hasAttribute(`autoload`)}get autoPause(){return this.hasAttribute(`autopause`)}get noCookie(){return this.hasAttribute(`nocookie`)}get posterQuality(){return this.getAttribute(`posterquality`)||`hqdefault`}get posterLoading(){return this.getAttribute(`posterloading`)||`lazy`}get params(){return`start=${this.videoStartAt}&${this.getAttribute(`params`)}`}set params(e){this.setAttribute(`params`,e)}set posterQuality(e){this.setAttribute(`posterquality`,e)}get disableNoscript(){return this.hasAttribute(`disablenoscript`)}setupDom(){let e=this.attachShadow({mode:`open`}),t=``;window.liteYouTubeNonce&&(t=`nonce="${window.liteYouTubeNonce}"`),e.innerHTML=`
      <style ${t}>
        :host {
